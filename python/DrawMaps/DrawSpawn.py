...
def draw_birth_map(iCiv, area, exceptions, folder, iPeriod=None):
    ...
    elif area == dPeriodCoreArea:
        identifier = iPeriod
        print(name)
        name_display = f"{name} -> {dPeriodNames.get(iPeriod, '').replace('_', ' ')}"
    ...
    for (x, y), iTerrainValue in terrain_values:
        if iTerrainValue != 0 and is_area(area, exceptions, identifier, (x, iWorldY-1-y)):
